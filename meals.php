...
switch ($action) {
    case 'add':
        ...
        $prompt = "Estime les valeurs nutritionnelles pour : "$description". " ."Réponds UNIQUEMENT avec ce JSON (entiers, pas de virgules) :" ."{"kcal":0,"protein":0,"carbs":0,"fat":0}";
    ...
    case 'analyze':
        ...
        $stmt->execute([$userId]);
        $targets = $stmt->fetch();
        if (!$targets) jsonResponse(false, [], 'Objectifs non trouvés');
        $kcal_pct = $targets['tdee_kcal'] > 0
            ? ($log['total_kcal'] / $targets['tdee_kcal']) * 100 : 0;
        $fat_pct = $log['total_kcal'] > 0
            ? ($log['total_fat_g'] * 9 / $log['total_kcal']) * 100 : 0;
        $status = 'Suffisant'; $title = 'Alimentation équilibrée';
        if ($kcal_pct < 80)  { $status='Insuffisant'; $title='Apport calorique trop faible'; }
        if ($kcal_pct > 115) { $status='Excessif';    $title='Apport calorique excessif'; }
        if ($fat_pct  > 35)  { $status='Trop gras';   $title='Alimentation trop riche en graisses'; }
        $summary = sprintf(
            "Vous avez consommé %d kcal sur un objectif de %d kcal (%.0f%%). " .
            "Protéines : %dg/%dg | Glucides : %dg/%dg | Lipides : %dg/%dg.",
        ...
        $tips = ['Mangez plus de légumes verts à chaque repas','Buvez au moins 2L d eau par jour','Évitez les sucres raffinés en soirée'];
        if ($kcal_pct < 80)  $tips[] = 'Ajoutez une collation riche en protéines';
        if ($fat_pct  > 35)  $tips[] = 'Réduisez les fritures, préférez vapeur/four';
        if ($kcal_pct > 115) $tips[] = 'Réduisez les portions, évitez les calories liquides';
        $stmt = $pdo->prepare("UPDATE daily_logs SET status = ? WHERE id = ?");
        $stmt->execute([$status, $log['id']]);
        jsonResponse(true, ['analysis' => ['status'  => $status,'title'   => $title,'summary' => $summary,'tips'    => $tips]]);
        break;
    default:
